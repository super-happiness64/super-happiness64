...
checkboxes.forEach((cb) => {
  cb.addEventListener('change', () => {
    ...
    cards.forEach((card) => {
      ...
      card.style.display = activeCategories.includes(category)
        ? 'flex'
        : 'none';
    });
  });
...
if (e.target.classList.contains('clear-cart-btn')) {
  if (cart.length === 0) return alert("No items to clear");
  sessionStorage.removeItem('nurseryCart');
  renderCartItems();
  return alert("Cart cleared");
}
...
if (contactForm) {
  contactForm.addEventListener('submit', function (e) {
    ...
    const customerData = {
      ...
      orderInfo:
        document.getElementById('custom-option')?.value || 'Custom Order',
